...
- Analytics page — Rating tab (distribution histogram, percentile bands, rating trend over time, per-category comparison, consistency leaderboard), ABV tab (distribution histogram, ABV vs. rating scatter plot, avg ABV by category), Geographic tab (sortable country ranking table, zoomable world map — countries shaded by how many rated drinks you have with a rating pip at each centre, wine/whiskey region markers sized by count — Old World vs New World breakdown, region leaderboard), Time & Pace tab (discovery pace, seasonal tasting patterns, tastings-per-category trend), Style & Variety tab (most common wine varieties/beer styles, with a grape/blend toggle for wine, and an undiscovered-styles list), Producer tab (top/most prolific producers leaderboard, producer consistency), Vintage tab (wine-only — best vintages leaderboard, age-at-tasting vs. rating scatter), Exploration tab (Best Of leaderboard ranked by weighted rating), and Value tab (price vs. rating scatter, best-value leaderboard, avg price by category/country); scoped by a global category filter with per-chart overrides; click a bar/tile/point/country/marker to jump to the filtered drink list, or to that drink's tasting history
- Hierarchical regions — enter a region as a path (`Loire Valley / Sancerre`, or deeper: `Bourgogne / Côte de Nuits / Vosne-Romanée`) and filter at any level: pick the parent to get every appellation beneath it, or a single appellation on its own. The drink table shows the most specific part; the admin form autocompletes from a curated taxonomy (`client/src/data/wine-regions.json`) and warns — but never blocks — on a region it doesn't recognize
- On the map, a region holding subregions shows one marker with the combined total, ringed with one arc per subregion. Hover it to list them — each with its own average and count — and click a row to open that subregion; zoom past 3× and the marker splits into its subregions on the map
- Wine/whiskey regions are auto-geocoded (via OpenStreetMap) the first time a drink with a new (country, region) pair is saved, so they appear as markers on the Geographic map. A result outside the country asked for is rejected, so a bad match can't plant a marker on the wrong continent. `server/scripts/backfill-region-coordinates.js --dry-run` audits and fills the whole collection, parent paths included
